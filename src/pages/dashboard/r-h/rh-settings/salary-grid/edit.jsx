...
export default function Page() {
  ...
  const newSalaryGrid = {
    ...salaryGrid,
    salary: parseFloat(salaryGrid?.salary || 0).toFixed(2),
    retenueIRG: parseFloat(salaryGrid?.retenueIRG || 0).toFixed(2),
    salary_position: parseFloat(salaryGrid?.contributory_salary || 0).toFixed(2),
    salary_position_retenue: parseFloat(salaryGrid?.post_salary || 0).toFixed(2),
    s_s_retenue: (parseFloat(salaryGrid?.contributory_salary || 0) * SOCIAL_SECURITY_RATE).toFixed(
      2
    ),
    salary_impos: (
      parseFloat(salaryGrid?.post_salary || 0) + parseFloat(salaryGrid?.sum_taxable || 0)
    ).toFixed(2),
    net_salary_payer: parseFloat(salaryGrid?.net_salary_payable || 0).toFixed(2),
