...
const ENDPOINT = endpoints.purchaseSupply.purchaseOperations.list;
...
export function useGetPurchaseOperations(params) {
  ...
  const memoizedValue = useMemo(
    () => ({
      purchaseOperations: data?.data?.records || [],
      purchaseOperationsCount: data?.data?.total || 0,

      purchaseOperationsLoading: isLoading,
      purchaseOperationsError: error,
      purchaseOperationsValidating: isValidating,
      purchaseOperationsEmpty: !isLoading && !isValidating && !data?.data?.records.length,
    }),
...
export async function getFiltredPurchaseOperations(params) {
...
export function useGetPurchaseOperation(id) {
  const url = id ? `${ENDPOINT}/${id}` : '';
  const { data, isLoading, error, isValidating } = useSWR(url, fetcher, swrOptions);
  
  const memoizedValue = useMemo(
    () => ({
      purchaseOperation: data?.data,
      purchaseOperationLoading: isLoading,
      purchaseOperationError: error,
      purchaseOperationValidating: isValidating,
      purchaseOperationEmpty: !isLoading && !isValidating && !data?.data,
    }),
 ...
 */
export function useGetPurchaseOperationItems(id, params) {
...
export async function getFiltredPurchaseOperationItems(id, params) {
  const url = id ? ENDPOINT_ITEMS(id) : null;
...
export async function confirmPurchaseOperation(id, data) { 
  const endpoint = ENDPOINT_CONFIRM(id);
  await axios.post(endpoint, data);
  mutate(ENDPOINT);
}

export async function cancelPurchaseOperation(id, data) { 
