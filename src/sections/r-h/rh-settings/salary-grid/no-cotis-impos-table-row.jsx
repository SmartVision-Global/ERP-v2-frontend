...
export function NoCotisImposTableRow({ row, selected, onDeleteRow, index, update, fields }) {
  const menuActions = usePopover();
  const confirmDialog = useBoolean();
  const { watch, control, setValue } = useFormContext();
  const watchedPercent = useWatch({
    control,
    name: `cotis_no_impos_items[${index}].percent`,
  });

  const watchedAmount = useWatch({
    control,
    name: `cotis_no_impos_items[${index}].amount`,
  });
  const values = watch();
