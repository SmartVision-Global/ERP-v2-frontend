...
import { NATURE } from './recovery-table-row';
...
export function RecoveryHistoryDialog({
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <Typography>{params.row?.data?.permanency_id}</Typography>
        </Box>
      ),
    },
    {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ gap: 0.5, display: 'flex', flexDirection: 'column' }}>
          {params.row?.data?.overtime_work_date ? (
            <>
              <span>{fDate(params.row?.data?.overtime_work_date)}</span>
              <Box component="span" sx={{ typography: 'caption', color: 'text.secondary' }}>
                {fTime(params.row?.data?.from_date)}
              </Box>
            </>
    ...
    },
    {
      ...
      hideable: false,
      renderCell: (params) => (
        <Label variant="soft" color={params.row?.data?.nature === '1' ? 'info' : 'default'}>
          {NATURE[params.row?.data?.nature]}
        </Label>
      ),
    },
    {
